...
#     output_dir = os.path.join(project_root, "test_dataset_json")
...
# =============================================================================
# 0. PROJECT PATHS (automatic, relative to this script or overridable via args)
# =============================================================================
# If you launch this as a Ghidra script you can optionally pass the project root
# as the first script-argument (getScriptArgs()). If not provided, the script
# autodetects the project root as the parent folder of the script file.

# Detect script location
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))

# Default project root = parent folder of the ghidra_scripts directory
PROJECT_ROOT = os.path.abspath(os.path.join(SCRIPT_DIR, ".."))

# If the user passed an explicit project root as first script argument, use it.
# (Useful when invoking headless or from outside.)
try:
    scripted_args = getScriptArgs()
except:
    scripted_args = []

if isinstance(scripted_args, (list, tuple)) and len(scripted_args) > 0:
    # allow the user to override project root from Ghidra script args
    PROJECT_ROOT = os.path.abspath(scripted_args[0])

# Input binaries directory and output JSON directory (both under project root)
INPUT_BINARIES_DIR = os.path.join(PROJECT_ROOT, "test_dataset_binaries")
OUTPUT_JSON_DIR = os.path.join(PROJECT_ROOT, "test_dataset_json")
...
def run_analysis():
    ...
    # --- Save JSON to OUTPUT_JSON_DIR (project-root/test_dataset_json) ---
    # Use the output folder we computed at the top. This makes paths relative
    # to the project root and independent from where the script is launched.
    ...
    output_file = os.path.join(output_dir, "{}_features.json".format(safe_name))
    ...
    try:
        ...
        print("[!] Failed to write JSON: {}".format(e))
