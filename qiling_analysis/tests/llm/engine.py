...
3. For PROPRIETARY_CRYPTO:
   - Provide a detailed technical analysis grounded in FACTS visible in STRACE.
   - Explain WHY the behavior does not match known algorithms.
   - Provide a descriptive label for crypto_algorithm:
       e.g. "proprietary_stream_cipher", "proprietary_xor_cipher", etc.

4. For STANDARD_CRYPTO:
   - Set crypto_algorithm to the exact matching algorithm name.
...
def run_engine(analysis_file: str, strace_file: str, out_file: str):
    ...
    print(f"[*] Loading analysis file: {analysis_file}")
    analysis_raw = load_text(analysis_file)  # includes YARA, Qiling, memory dumps, etc.
    ...
    final_output = {
        ...
        # RAW analysis preserved exactly
        "analysis_section": analysis_raw,

        # LLM-evaluated STRACE JSON
        "strace_section": strace_result
    }

    # Write final JSON file
    Path(out_file).write_text(json.dumps(final_output, indent=2), encoding="utf-8")
    print(f"[✓] Final combined report saved → {out_file}")
...
def main():
    ...
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--analysis", help="Analysis log file (preferred)")
    group.add_argument("--input", help="Alias for analysis log file")
