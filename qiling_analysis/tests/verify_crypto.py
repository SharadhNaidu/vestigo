...
def run_binary_with_hooks(binary_path, crypto_funcs, rootfs_path, filename, constant_results):
    ...
    try:
        ...
        # ADAPTIVE Confidence Scoring (0-100 scale) - Context-Aware & Flexible
        # This system adapts weights based on what evidence is available
        reasons = []
        
        # Determine if we have YARA results (from global scope via logger)
        yara_detected = []
        if logger and 'yara' in logger.data:
            yara_detected = logger.data['yara'].get('detected', [])

        # Prepare per-factor points with ADAPTIVE weights
        f1_points = 0  # constants (0-40, or boosted if no other evidence)
        f2_points = 0  # function names (0-40, adaptive based on quantity & quality)
        f3_points = 0  # crypto loops (0-20)
        f4_points = 0  # crypto-op ratio (0-15)
        f5_points = 0  # avalanche (0-15, not penalized if unavailable)
        f6_points = 0  # YARA detection (0-30, NEW!)
        
        # Evidence availability flags
        has_constants = bool(constant_results)
        has_functions = bool(crypto_funcs)
        has_loops = bool(crypto_loops)
        has_yara = bool(yara_detected)
        has_execution = total_instructions > 0
        
        # Count available evidence sources
        evidence_sources = sum([has_constants, has_functions, has_loops, has_yara, has_execution])
        
        # === FACTOR 1: Crypto Constants (Adaptive: 0-40 pts) ===
        ...
        elif 'RSA' in constant_results:
            # RSA exponents found (weaker but still evidence)
            f1_points = 15
            reasons.append("RSA constants detected (exponents/moduli)")
        
        # === FACTOR 2: Function Names (Adaptive: 0-40 pts, boosted if primary evidence) ===
        ...
        strong_crypto_names = ['aes', 'des', 'rsa', 'sha', 'md5', 'encrypt', 'decrypt', 
                               ...
                               'chacha', 'poly1305', 'gcm', 'cbc', 'ecb', 'ctr']
        
        strong_matches = sum(1 for name, _ in crypto_funcs 
                            if any(pattern in name.lower() for pattern in strong_crypto_names))
        
        # Adaptive scoring: if we have many function names but no other evidence, boost weight
        if strong_matches >= 10:
            # Many strong crypto function names = likely legitimate crypto library
            if evidence_sources <= 2:  # Limited other evidence
                f2_points = 40  # BOOSTED weight
                reasons.append(f"{strong_matches} strong crypto function names (library)")
            else:
                f2_points = 35
                reasons.append(f"{strong_matches} strong crypto function names")
        elif strong_matches >= 5:
            if evidence_sources <= 2:
                f2_points = 35  # BOOSTED
                reasons.append(f"{strong_matches} strong crypto function names")
            else:
                f2_points = 30
                reasons.append(f"{strong_matches} crypto function names")
        ...
        # === FACTOR 3: Crypto Loops (0-20 pts) ===
        if len(crypto_loops) >= 10:
            f3_points = 20
            reasons.append(f"{len(crypto_loops)} crypto loops (strong round functions)")
        elif len(crypto_loops) >= 3:
            f3_points = 15
        ...
        # === FACTOR 4: Crypto-Op Ratio (0-15 pts) ===
        if ratio > 0.10:
            f4_points = 15
        ...
        # === FACTOR 5: Avalanche Effect (0-15 pts, NOT penalized if unavailable) ===
        # If avalanche test wasn't possible (no I/O captured), don't penalize
        if avalanche_detected:
            f5_points = 15
            reasons.append("Avalanche effect confirmed")
        elif total_captures == 0:
            # Avalanche test couldn't run - redistribute weight to other factors
            # This prevents unfair penalty for libraries or stripped binaries
            f5_points = 0  # No penalty
            if has_functions and strong_matches >= 5:
                # Boost function name weight slightly to compensate
                bonus = 5
                f2_points = min(f2_points + bonus, 40)
                reasons.append("Avalanche test unavailable (functions inlined/not called)")
        
        # === FACTOR 6: YARA Static Detection (NEW: 0-30 pts) ===
        if has_yara:
            yara_count = len(yara_detected)
            if yara_count >= 2:
                f6_points = 30
                reasons.append(f"YARA detected {yara_count} algorithms: {', '.join(yara_detected[:3])}")
            elif yara_count == 1:
                f6_points = 25
                reasons.append(f"YARA detected: {yara_detected[0]}")
        
        # === CALCULATE TOTAL with ADAPTIVE CEILING ===
        # Base scoring
        raw_score = f1_points + f2_points + f3_points + f4_points + f5_points + f6_points
        
        # Adaptive confidence: if multiple strong indicators, boost confidence
        if (f1_points >= 30 or f2_points >= 30 or f6_points >= 25) and evidence_sources >= 2:
            # Strong primary evidence + supporting evidence = HIGH confidence
            confidence_score = min(raw_score + 10, 100)  # Bonus boost
        else:
        ...
        # Print transparent breakdown
        print("\n[*] ADAPTIVE Confidence Scoring (0-100):")
        print(f"    - Constants evidence : {f1_points} pts (0-40)")
        print(f"    - Function names     : {f2_points} pts (0-40, adaptive)")
        print(f"    - Crypto loops       : {f3_points} pts (0-20)")
        print(f"    - Crypto-op ratio    : {f4_points} pts (0-15)")
        print(f"    - Avalanche effect   : {f5_points} pts (0-15, optional)")
        print(f"    - YARA detection     : {f6_points} pts (0-30)")
        print(f"    -> RAW TOTAL: {f1_points + f2_points + f3_points + f4_points + f5_points + f6_points}")
        print(f"    -> FINAL SCORE: {confidence_score}/100  => {confidence}")
