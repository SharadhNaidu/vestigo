# #!/usr/bin/env python3
# """
# Script to convert JSON files from ghidra_json folder to CSV format.
# Extracts features for each function and saves to CSV with all required columns.
# """
...
#     Examples: monocypher, tinycrypt_aes_decrypt, etc.
#     """
...
#     """
#     # Most files don't have compiler in name, defaulting to gcc
#     # Can be extended if compiler info is encoded in filename
...
#     # Node-level aggregated features (from first node or aggregated)
...
#     json_dir = script_dir / 'ghidra_json'
#     output_csv = script_dir / 'ghidra_features.csv'
...
Script to convert JSON files from ghidra_json folder to CSV format.
Extracts features for each function and saves to CSV with all required columns.
Adds label_final classification based on your mapping.
"""
...
# ================================================================
# FINAL LABEL MAPPING (as requested)
# ================================================================
LABEL_MAP = {
    # AES
    "AES": "AES",
...
def map_to_label_final(label):
    """Return final grouped label."""
    return LABEL_MAP.get(label, "Non-Crypto")
...
def process_function(function_data, metadata):
    ...
    node_list = function_data.get('node_level', [])
    if node_list:
        total_instruction_count = sum(n.get('instruction_count', 0) for n in node_list)
        total_crypto_hits = sum(n.get('crypto_constant_hits', 0) for n in node_list)
        total_bitwise_density = sum(n.get('bitwise_op_density', 0) for n in node_list)
        total_immediate_entropy = sum(n.get('immediate_entropy', 0) for n in node_list)
        table_lookup = any(n.get('table_lookup_presence', False) for n in node_list)
        num_nodes = len(node_list)

        features['instruction_count'] = total_instruction_count
        features['immediate_entropy'] = total_immediate_entropy / num_nodes
        features['bitwise_op_density'] = total_bitwise_density / num_nodes
        features['table_lookup_presence'] = 1 if table_lookup else 0
        features['crypto_constant_hits'] = total_crypto_hits

        edges = function_data.get('edge_level', [])
        if edges:
            total_complexity = sum(e.get('branch_condition_complexity', 0) for e in edges)
            features['branch_condition_complexity'] = total_complexity / len(edges)
        else:
            features['branch_condition_complexity'] = 0
    else:
        features['instruction_count'] = 0
    ...
    sha_k_hits = adv.get('sha_k_table_hits', 0)
    sha_init_hits = adv.get('sha_init_constants_hits', 0)
    features['has_sha_constants'] = 1 if (sha_k_hits or sha_init_hits) else 0
    ...
    features['function_address'] = function_data.get('address', '0')
    features['label'] = function_data.get('label', 'Unknown')
    features['label_final'] = map_to_label_final(features['label'])
...
def convert_json_to_csv(json_dir, output_csv):
    ...
    csv_columns = [
        ...
        'function_name', 'function_address', 'label', 'label_final',
        ...
        'avg_edge_branch_condition_complexplexity', 'has_aes_sbox',
        'rsa_bigint_detected', 'has_aes_rcon', 'has_sha_constants',
    ...
    for json_file in json_files:
        ...
        filename = json_file.name
        arch = extract_architecture_from_filename(filename)
        algo = extract_algorithm_from_filename(filename)
        compiler = extract_compiler_from_filename(filename)
        opt = extract_optimization_from_filename(filename)
...
def main():
    script_dir = Path(__file__).parent
    json_dir = script_dir / "ghidra_json"
    output_csv = script_dir / "ghidra_features_labeled.csv"
