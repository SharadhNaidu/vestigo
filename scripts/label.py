    ...
    try:
        ...
        while True:
            ...
            if brace_count == 0:
                ...
                if signatures:
                    # Filter out short signatures (less than 5 bytes) to avoid false positives
                    # especially for common integers like 0x03, 0x01000100
                    filtered_signatures = [s for s in signatures if len(s) >= 5]
...
def process_files():
    ...
    if os.path.isfile(target_dir):
        ...
        target_dir = os.path.dirname(target_dir) # For negative check context
        ...
        print(f"Path {target_dir} not found.")
        return
    ...
    for file_path in files_to_process:
        ...
        try:
                    ...
                    if is_negative:
                        new_label = "non-crypto"
                            ...
                            for node in func.get("node_level", []):
                                ...
                                # Collect constant flags
                                flags = node.get("constant_flags", {})
                                if flags:
                            ...
                            elif "AES_SBOX_BYTES" in all_flags or "AES_SBOX_INTS" in all_flags:
                                new_label = "AES"
                            elif "DES_SBOX" in all_flags:
                                new_label = "DES"
                            elif "SHA1_CONSTANTS" in all_flags:
                                new_label = "SHA-1"
                            elif "SHA256_CONSTANTS" in all_flags:
                                new_label = "SHA-256"
                            elif "SHA512_CONSTANTS" in all_flags:
                                new_label = "SHA-512"
                            elif "MD5_CONSTANTS" in all_flags:
                                new_label = "MD5"
                            else:
            ...
            if updated:
                ...
                print(f"Updated {file} (Label changed)")
                count += 1
